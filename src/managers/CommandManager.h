...
public:
	...
	static int RegisterCommand(std::vector<std::string> names, const AdminFlag flag, std::function<int(Player*, std::vector<std::string>)>);
	...
	AdminFlag GetFlag(std::string name);
...
private:
	...
	inline static std::unordered_map<std::string, AdminFlag> commandFlag;
};